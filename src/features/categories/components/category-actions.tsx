...
export function CategoryActions({ category }: CategoryActionsProps) {
  ...
  const handleToggle = () => {
    startTransition(async () => {
      const { error } = await toggleCategoryActive(category.id, category.is_active)
      if (error) {
        alert(error)
      }
    })
      ...
      <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Esta acción eliminará la categoría <strong>{category.name}</strong>. 
              Si hay herramientas asociadas a esta categoría, la eliminación será rechazada.
            </DialogDescription>
          </DialogHeader>
